...
const User = () => {
  ...
  const applyRolePreset = () => {
    setForm((prev) => ({
      ...prev,
      permission: [...(rolePresets[prev.role] || [])],
    }));
  };
...
{users.map((u, idx) => (
                <tr key={u.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`px-2 py-1 rounded-full text-xs font-medium ${u.role === "admin" ? "bg-purple-100 text-purple-700" : "bg-green-100 text-green-700"}`}
                    >
                      {u.role}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 flex gap-2">
                    ...
                    {p("user.delete") && u.id !== currentUser?.id && (
                      <Button
                        size="sm"
                        variant="danger"
          ...
          {users.map((u) => (
            ...
            >
              <div className="flex justify-between items-start">
                ...
                <span
                  className={`px-2 py-0.5 rounded-full text-xs font-medium ${u.role === "admin" ? "bg-purple-100 text-purple-700" : "bg-green-100 text-green-700"}`}
                >
                  {u.role}
                </span>
              </div>
              ...
              <div className="flex gap-2 pt-1">
                {p("user.update") && (
                  <Button size="sm" variant="info" onClick={() => openEdit(u)}>
                    Edit
                  </Button>
                )}
                {p("user.delete") && u.id !== currentUser?.id && (
